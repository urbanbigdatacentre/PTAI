...
# install_packages.R - Install all required R packages for the analysis
...
install_safe <- function(packages, from_github = FALSE, optional = FALSE) {
  for (pkg in packages) {
    cat('Installing', pkg, '...\n')
    success <- tryCatch({
      if (from_github) {
        # Try with different methods for GitHub packages
        tryCatch({
          remotes::install_github(pkg, dependencies = TRUE, upgrade = 'never', force = TRUE)
        }, error = function(e1) {
          cat('Retrying with devtools...\n')
          devtools::install_github(pkg, dependencies = TRUE, upgrade = 'never', force = TRUE)
        })
      } else {
      ...
      if (optional) {
        cat('⚠️ ', pkg, 'is optional, continuing with build...\n')
        return(FALSE)
      } else {
